...
public final class CosmeticsClient {
    ...
    public static void init() {
        ...
        ScreenEvents.AFTER_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
            ...
            Screens.getButtons(screen).add(ButtonWidget.builder(Text.literal("Косметика"),
    ...
    private static void drawHalo(Vec3d p, boolean demon) {
        int main = demon
                ? ColorHelper.fromFloats(0.95f, 0.12f, 0.18f, 0.92f)
                : ColorHelper.fromFloats(1.0f, 0.88f, 0.35f, 0.96f);
        int glow = demon
                ? ColorHelper.fromFloats(0.35f, 0.02f, 0.04f, 0.55f)
                : ColorHelper.fromFloats(1.0f, 0.98f, 0.75f, 0.55f);
        double cy = p.y + 2.38;
        int segments = 18;
    ...
    private static final class WardrobeScreen extends Screen {
        ...
        private List<Cosmetic> filtered() {
            ...
            return COSMETICS.stream().filter(c -> c.category == category())
                    .filter(c -> q.isBlank() || c.name.toLowerCase().contains(q)).toList();
        }
        ...
        @Override public void render(DrawContext c, int mx, int my, float delta) {
            ...
            c.drawText(textRenderer, Text.literal(ClientConfig.isPlus() ? "Plus • расширенная коллекция" : "Free • базовая коллекция"), l + 22, t + 38, ClientConfig.hudColor(), false);
            int currencyX = r - 110;
            ...
            int contentLeft = l + 184, searchL = contentLeft, searchR = contentLeft + 325;
            ...
            int cardW = 142, cardH = 132, gap = 10, startY = t + 96;
            for (int i = 0; i < items.size(); i++) {
                int col = i % 3, row = i / 3;
                int x = contentLeft + col * (cardW + gap), y = startY + row * (cardH + gap);
        ...
        private void drawCard(DrawContext c, Cosmetic item, int x, int y, int w, int h, int mx, int my) {
            ...
            boolean locked = item.plusOnly && !ClientConfig.isPlus();
            ...
            if (locked) {
                c.fill(x + 8, y + 94, x + w - 8, y + 116, 0xFF302732);
                c.drawText(textRenderer, Text.literal("PLUS • ПОДПИСКА"), x + 20, y + 101, 0xFFFFD36E, false);
            } else {
                String state = equipped ? "НАДЕТО" : "НАДЕТЬ";
                c.drawText(textRenderer, Text.literal(state), x + 10, y + 100, equipped ? 0xFF69E6A6 : ClientConfig.hudColor(), false);
            }
        }
        ...
        private void drawPreview(DrawContext c, int x, int y, int w, int h) {
            ...
            if (CosmeticsState.isEquipped("angel_halo")) drawPreviewHalo(c, cx, y + 20, false);
            if (CosmeticsState.isEquipped("demon_halo")) drawPreviewHalo(c, cx, y + 20, true);
        ...
        private void drawPreviewHalo(DrawContext c, int cx, int cy, boolean demon) {
            int color = demon ? 0xFFF02A45 : 0xFFFFE477;
            int glow = demon ? 0x667A1020 : 0x66FFF6A0;
            for (int i = 0; i < 16; i++) {
                double a = Math.PI * 2 * i / 16.0;
                int px = cx + (int)(Math.cos(a) * 22);
                int py = cy + (int)(Math.sin(a) * 7);
                c.fill(px - 3, py - 2, px + 4, py + 3, i % 3 == 0 ? glow : color);
            }
            if (demon) {
                c.fill(cx - 18, cy - 1, cx - 11, cy - 14, color);
                c.fill(cx + 11, cy - 1, cx + 18, cy - 14, color);
            }
        }
        ...
        @Override public boolean mouseClicked(Click click, boolean doubled) {
            ...
            for (int i = 0; i < Category.values().length; i++) {
                int yy = t + 72 + i * 42;
                if (mx >= l + 12 && mx <= l + 158 && my >= yy - 5 && my <= yy + 30) { selected = i; return true; }
            }
            int contentLeft = l + 184, searchL = contentLeft, searchR = contentLeft + 325;
            if (mx >= searchL && mx <= searchR && my >= t + 54 && my <= t + 82) { searchFocused = true; return true; }
            List<Cosmetic> items = filtered(); int cardW = 142, cardH = 132, gap = 10, startY = t + 96;
            for (int i = 0; i < items.size(); i++) {
                int col = i % 3, row = i / 3;
                int x = contentLeft + col * (cardW + gap), y = startY + row * (cardH + gap);
                if (y + cardH > b - 58) break;
                if (mx >= x && mx <= x + cardW && my >= y && my <= y + cardH) {
                    Cosmetic item = items.get(i);
                    if (item.plusOnly && !ClientConfig.isPlus()) return true;
                    CosmeticsState.toggle(item.id);
                    return true;
                }
            }
        ...
        @Override public boolean keyPressed(KeyInput input) {
            if (searchFocused && input.key() == GLFW.GLFW_KEY_BACKSPACE) { if (!search.isEmpty()) search = search.substring(0, search.length() - 1); return true; }
