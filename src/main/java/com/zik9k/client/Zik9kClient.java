...
public final class Zik9kClient implements ClientModInitializer {
    ...
    @Override public void onInitializeClient() {
        ...
        TargetEspWorldRenderer.register();
    ...
    private static final class ClickGuiScreen extends Screen {
        ...
        @Override public boolean mouseClicked(Click click, boolean doubled){
            double mx=click.x(), my=click.y(); int button=click.button();int l=panelLeft(),t=panelTop(),r=l+PANEL_WIDTH,b=t+PANEL_HEIGHT;
            if(button==1){List<Module> mods=ModuleManager.getModules(ModuleCategory.values()[selectedTab]).stream().filter(this::matches).toList();int cy=t+104;int cx=l+SIDEBAR_WIDTH+23;for(Module m:mods){if(cy>b-28)break;if(mx>=cx&&mx<=r-20&&my>=cy&&my<=cy+48){if(m instanceof TriggerBotModule tb){client.setScreen(new TriggerBotSettingsScreen(tb,this));return true;}if(m instanceof ESPModule e){client.setScreen(new ESPSettingsScreen(e,this));return true;}if(m instanceof BlockOverlayModule bo){client.setScreen(new BlockOverlaySettingsScreen(bo,this));return true;}if(m instanceof TracersModule tr){client.setScreen(new TracersSettingsScreen(tr,this));return true;}if(m instanceof KillAuraModule ka){client.setScreen(new KillAuraSettingsScreen(ka,this));return true;}return true;}cy+=58;}return super.mouseClicked(click,doubled);}
            if(button!=0)return super.mouseClicked(click,doubled);if(mx>=l+10&&mx<=l+SIDEBAR_WIDTH-10){for(int i=0;i<TABS.length;i++){int tt=t+112+i*48;if(my>=tt-8&&my<=tt+28){selectedTab=i;searchFocused=false;return true;}}int st=b-47;if(my>=st-6&&my<=st+24){client.setScreen(new SettingsScreen());return true;}}if(mx>=l+12&&mx<=l+52&&my>=t+54&&my<=t+94){avatarIndex=(avatarIndex+1)%AvatarTextures.count();ClientConfig.setAvatarIndex(avatarIndex);return true;}int sl=r-210,sr=r-20;if(mx>=sl&&mx<=sr&&my>=t+13&&my<=t+33){searchFocused=true;lastBlinkTime=System.currentTimeMillis();cursorVisible=true;return true;}List<Module> mods=ModuleManager.getModules(ModuleCategory.values()[selectedTab]).stream().filter(this::matches).toList();int cy=t+104,cx=l+SIDEBAR_WIDTH+23;for(Module m:mods){if(cy>b-28)break;if(mx>=cx&&mx<=r-20&&my>=cy&&my<=cy+48){m.toggle();return true;}cy+=58;}searchFocused=false;return super.mouseClicked(click,doubled);}
        @Override public boolean charTyped(CharInput input){if(searchFocused&&input.isValidChar()&&searchQuery.length()<48){String text=input.asString();if(searchQuery.length()+text.length()<=48){searchQuery+=text;lastBlinkTime=System.currentTimeMillis();cursorVisible=true;return true;}}return super.charTyped(input);}
        @Override public boolean keyPressed(KeyInput input){int key=input.key(),mods=input.modifiers();if(searchFocused){if(key==GLFW.GLFW_KEY_ESCAPE){searchFocused=false;return true;}if(key==GLFW.GLFW_KEY_BACKSPACE){if(!searchQuery.isEmpty())searchQuery=searchQuery.substring(0,searchQuery.length()-1);return true;}if(key==GLFW.GLFW_KEY_DELETE||key==GLFW.GLFW_KEY_A&&(mods&GLFW.GLFW_MOD_CONTROL)!=0){searchQuery="";return true;}}return super.keyPressed(input);}
